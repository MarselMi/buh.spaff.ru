...
def get_transaction_holder(pk):
    ...
    try:
        with conn.cursor() as cursor:
            response = \
                f'''
                SELECT `status`, `create_date`, `type_transaction`, `transaction_date`, `name`, `mainapp_balanceholder`.`organization_holder` AS `balance_holder_id`, `amount`, `mainapp_paytype`.`pay_type` AS `type_payment_id`, CONCAT(`authapp_customuser`.`last_name`,' ', `authapp_customuser`.`first_name`) AS `author_id`, `check_img`
